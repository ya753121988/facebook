...
async def youtube_dl_call_back(_bot, update):
    # Constants
    AD_STRING_TO_REPLACE = "please report this issue on https://github.com/LazyDeveloperr"
    ...
    save_ytdl_json_path = (
        DOWNLOAD_LOCATION + "/" + str(update.from_user.id) + f"{ranom}" + ".json"
    )
    ...
    tmp_directory_for_each_user = (
        DOWNLOAD_LOCATION + "/" + str(update.from_user.id) + f"{random1}"
    )
    ...
    if t_response:
        ...
        if file_size > TG_MAX_FILE_SIZE:
            await update.message.edit_caption(
                caption=Script.RCHD_TG_API_LIMIT.format(
            ...
            await update.message.edit_caption(
                caption=Script.AFTER_SUCCESSFUL_UPLOAD_MSG_WITH_TS.format(
